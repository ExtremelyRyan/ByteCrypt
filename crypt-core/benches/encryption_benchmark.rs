...
pub fn dracula_content_encryption(c: &mut Criterion) {
    ...
    let contents = get_vec_file_bytes(DRACULA).unwrap_or(vec![]);
...
pub fn shakespeare_content_encryption(c: &mut Criterion) {
    ...
    let b: [u8; 32] = [0u8; 32];
    let fc = FileCrypt::new(s.clone(), s, "".to_string(), pb, b);
    let contents = get_vec_file_bytes(SHAKESPEARE).unwrap_or_default();
...
pub fn test_zip(c: &mut Criterion) {
    let contents = get_vec_file_bytes(DRACULA).unwrap_or_default();
